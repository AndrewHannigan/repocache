...
The clone isn't free. A workspace clones from the real upstream, so creating one needs the network and working auth — `--reference` makes the object transfer nearly free, but the ref exchange still happens — and its remote-tracking refs only refresh when *it* fetches, not when the store does. A worktree's creation is offline and instant, and a single store fetch updates every tree at once; for long-lived trees those matter more, for short-lived agent scratch the clean isolation wins.

Because every workspace borrows the store's objects through alternates, those objects are load-bearing for it: shed sets `gc.auto = 0` on the store so a background prune can't delete an object a live workspace is using (a hazard worktrees avoid — git's gc is worktree-aware), and holds a per-repo `flock` so sync and workspace creation can't race. For the same reason the store has to stay put: moving or deleting it breaks the workspaces pointing into it, exactly as it would break a worktree.

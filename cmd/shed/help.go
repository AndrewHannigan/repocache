...
Keeps a read-only mirror of your git repos and hands your agents isolated,
writable workspaces to make changes in — so multiple agents can share the
...
The whole loop:
    ...
    # ... your agent takes it from here: reads the mirror, and on demand ...
...
cache), use 'shed uninstall'.
`,

	"uninstall": `uninstall — reverse agent integration

Removes the entries 'shed init' added to each agent's config:
...
that older shed versions installed.

Uses a sidecar state file
(~/.shed/agents.state.json) to know exactly which
entries are shed's, so user-added entries in the same files are
preserved.
...
Three lock scopes:
  ...
  per-cache-repo  <cache>/.git/shed.lock

...
| Antigravity CLI | `~/.gemini/antigravity-cli/` exists | `~/.gemini/settings.json` | session-context + bg-sync |
| opencode | `~/.config/opencode/` exists | n/a — plugin file (§8.9) | n/a — handled inside the plugin (§8.9) |

Antigravity is a Gemini-CLI fork: it shares the `~/.gemini` config dir with
the standalone Gemini CLI and reads its `includeDirectories` and SessionStart
hooks from `~/.gemini/settings.json`. Because `~/.gemini` alone is ambiguous,
detection keys off the `~/.gemini/antigravity-cli/` app-data subdir the
Antigravity CLI creates on first run.

opencode is the exception to the table above: it has no SessionStart
shell-command hook and no path allowlist, so it does not edit a settings
file at all. Its integration is a dropped-in plugin (§8.9). The remaining
subsections (§8.3–§8.7) describe the settings-file agents; §8.9 covers
opencode.
...
| Antigravity CLI | `~/.gemini/settings.json` | `includeDirectories` |
...
### 8.6 SessionStart hooks (Claude, Codex, Antigravity)

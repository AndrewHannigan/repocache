...
| opencode | `~/.config/opencode/` exists | n/a — plugin file (§8.8) | n/a — handled inside the plugin (§8.8) |

opencode is the exception: it has no SessionStart
shell-command hook and no path allowlist, so it does not edit a settings
file at all. Its integration is a dropped-in plugin (§8.8). Subsections
...
Claude and Codex (§8.6), and the plugin's load-time call for opencode
(§8.8). The `--agent` flag selects
...
the JSON envelope does. opencode (§8.8) is plugin-based: it pushes the text
...
opencode has no per-directory allowlist, so repocache registers no paths for
it; its read/grep tools already reach absolute paths, and the `chmod a-w` on
`repos/` enforces read-only.

### 8.5 Marker convention for idempotent edits

Every entry repocache adds must be identifiable for clean uninstall:

- **JSON/JSONC** (Claude): wrap repocache entries in a sentinel comment block:
  ```jsonc
...
`session-context`; the bg-sync entry is identical but for the command and
the Codex `statusMessage`).
...
### 8.8 opencode (plugin-based integration)
...
- Agent integration for Claude Code, Codex CLI, opencode
...
7. ✅ `pkg/agents/codex.go` + auto-detect

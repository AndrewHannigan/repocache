...
| Antigravity CLI | `~/.antigravity/` exists | `~/.antigravity/settings.json` | session-context + bg-sync |
...
| Antigravity CLI | `~/.antigravity/settings.json` | `includeDirectories` |
...
- **guide injection**: at load it snapshots `repocache session-context
...
`session-context --text` is a variant of §8.3 that prints just the
